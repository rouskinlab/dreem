...
def demultiplexed_files_test_fun(df, output_folder, sample_name):
   assert os.path.exists(os.path.join(output_folder, sample_name)), 'Output folder does not exist'
   for i, row in df.iterrows():
      assert os.path.exists(os.path.join(directory, sample_name, row['construct']+'_R1.fastq')), 'Demultiplexed file does not exist'
      assert len(open(os.path.join(directory, sample_name, row['construct']+'_R1.fastq')).readlines()) == num_reads*4, 'Demultiplexed file does not have the correct number of reads'
      with open(os.path.join(directory, sample_name, row['construct']+'_R1.fastq')) as f:
         assert f.readline()[:6+barcode_length] == '@READ:'+row['construct'], 'Demultiplexed file does not have the correct header'
         line = f.readline()
         assert len(line) == length_sequence+1, 'Demultiplexed file does not have the correct sequence length'
         assert line[row['barcode_start']:row['barcode_start']+barcode_length] == row['barcode_sequence'], 'Demultiplexed file does not have the correct barcode sequence'
         assert f.readline() == '+\n', 'Demultiplexed file does not have the correct header'
         assert len(f.readline()) == length_sequence+1, 'Demultiplexed file does not have the correct quality length'
         f.close()
      assert os.path.exists(os.path.join(directory, sample_name, row['construct']+'_R2.fastq')), 'Demultiplexed file does not exist'
      assert len(open(os.path.join(directory, sample_name, row['construct']+'_R2.fastq')).readlines()) == num_reads*4, 'Demultiplexed file does not have the correct number of reads'
