...
def add_library_info(df, df_library, verbose=False):
    ...
    assert 'reference' in list(df_library.index), "reference is not in library.csv"

    df_library.reference = df_library.reference.astype(str)
    if not sum([c in df_library.index.tolist() for c in ['section_start', 'section_end']]) == 2:
        df = pd.merge(df, df_library, on='reference', how='left')
    else:
        new_df = pd.DataFrame()
        assert len(df['reference'].unique()) == len(df), 'references are not unique'
        if verbose:
            iter_fun = lambda x: tqdm(x.groupby('reference'), total=len(df_library['reference'].unique()),
                                      desc='references')
        else:
        ...
        for idx, g in iter_fun(df_library):
            ...
            for _, row in g.iterrows():
                if row['reference'] not in df['reference'].tolist():
                    ...
                    continue
                row = pd.concat([row, df[df['reference'] == row['reference']][
                    [c for c in df.index if c not in row.index]].iloc[0]])
                if 'Unnamed: 0' in row.index:
                    row = row.drop('Unnamed: 0')
                if not 'section' in df_library.index:
                ...
                if isnan(row['section_start']) or isnan(row['section_end']):
                    ...
                    row['section_start'], row['section_end'] = int(row['section_start']), int(
                        row['section_end'])  # CAST TO INT
                    for c in ['sequence'] + [col for col in df.index.tolist() if
                                             (col != 'sub_hist' and type(df[col].iloc[0]) == list)]:
