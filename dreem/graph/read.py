...
from ..table.load import (RelReadTableLoader, MaskReadTableLoader,
...
def run(table: tuple[str, ...],
    ...
    writers = list(map(ReadGraphWriter, find_tables(table)))
...
class ReadGraphWriter(GraphWriter):
...
class ReadHistogram(OneSampleRefGraph, ABC):
    """ Histogram of the attributes of reads. """
...
class FieldReadHistogram(ReadHistogram, ABC):
    """ Histogram with multiple types of bit fields. """
    ...
    def graph_filename(self):
        ...
        by = "single" if self.data_type() is pd.Series else "grouped"
        return f"read-hist_of_{sort_codes}_by_{by}_{self.yattr()}".lower()

    @classmethod
    @abstractmethod
    def get_table_data(cls, table: RelReadTableLoader | MaskReadTableLoader,
                       field_codes: str) -> pd.DataFrame | pd.Series:
        """ Load all fields given by `field_codes` from the table. """
        return

    @classmethod
    def from_table(cls, table: RelReadTableLoader | MaskReadTableLoader,
                   field_codes: str):
        return cls(data=cls.get_table_data(table, field_codes),
