...
def check_fastqc_exists():
    ...
    assert validate_all_conditions(fastqc_installed_version, version), 'fastqc version is not correct: {} incompatible with {}'.format(fastqc_installed_version, version_txt)
...
def check_cutadapt_exists():
    ...
    assert validate_all_conditions(cutadapt_installed_version, version), 'cutadapt version is not correct: {} incompatible with {}'.format(cutadapt_installed_version, version_txt)
...
def check_bowtie2_exists():
    ...
    assert validate_all_conditions(bowtie2_installed_version[2], version), 'bowtie2 version is not correct {} incompatible with {}'.format(bowtie2_installed_version[2], version_txt)
...
def check_samtools_exists():
    ...
    assert validate_all_conditions(samtools_installed_version, version), 'samtools version is not correct: {} incompatible with {}'.format(samtools_installed_version, version_txt)
...
def check_rnastructure_exists(path=None):
    ...
    assert validate_all_conditions(rnastructure_installed_version.split(' ')[2], version), 'RNAstructure version is not correct: {} incompatible with {}'.format(rnastructure_installed_version.split(' ')[2], version_txt)
