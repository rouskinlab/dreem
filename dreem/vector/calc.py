...
========
                muts[ref_idx] = encode_match(read.seq[read_idx],
                                             read.qual[read_idx],
                                             min_qual)
>>>>>>>> 4764391 (Vectorize only full references):dreem/vector/calc.py
                ref_idx += 1
                read_idx += 1
...
========
    muts1 = muts.copy()
    vectorize_read(read1, muts1, ref_seq, length, qmin, ambid)
    # Vectorize read 2.
    muts2 = muts.copy()
    vectorize_read(read2, muts2, ref_seq, length, qmin, ambid)
